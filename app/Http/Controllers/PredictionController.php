...
class PredictionController extends Controller
{
    ...
    private static function visualize($road_network, $prediction_data) {
    ...
    private static function getPredictionData($road_network, $timestamp) {
        ...
        return array_values(array_filter(explode(' ', $response)));
    }
    ...
    public static function PEMSPrediction(Request $request) {
        $prediction_data = self::getPredictionData('pems', date('Y-m-d h:i:s', strtotime($request->input('date'))));
        ...
        $return['image'] = self::visualize($road_network, $prediction_data);
    ...
    public static function METRPrediction(Request $request) {
        $prediction_data = self::getPredictionData('metr', date('Y-m-d h:i:s', strtotime($request->input('date'))));
