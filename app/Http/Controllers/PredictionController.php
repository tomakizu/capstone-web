...
class PredictionController extends Controller
{
    ...
    public static function PEMSPrediction(Request $request) {
        ...
        curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode(['dataset' => 'pems', 'timestamp' => date('Y-m-d h:i:s', strtotime($request->input('date')))]));
        curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json'));
        $response = trim(curl_exec($ch), '[]');
        curl_close($ch);
        $prediction_data = array_values(array_filter(explode(' ', $response)));
        ...
        for ($i = 0; $i < count($prediction_data); $i++) {
    ...
    public static function METRPrediction(Request $request) {
        ...
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, 1);
        curl_setopt($ch, CURLOPT_POST, 1);
        curl_setopt($ch, CURLOPT_POSTFIELDS, json_encode(['dataset' => 'metr', 'timestamp' => date('Y-m-d h:i:s', strtotime($request->input('date')))]));
        curl_setopt($ch, CURLOPT_HTTPHEADER, array('Content-Type: application/json'));
        $response = trim(curl_exec($ch), '[]');
        curl_close($ch);
        $prediction_data = array_values(array_filter(explode(' ', $response)));
        ...
        $current_timestamp = time();
        $image->save(public_path('image/METR_' . $current_timestamp . '.jpg'));

        // prepare return data
        $return = array();
        $return['date'] = $request->date;
        $return['data'] = count($prediction_data);
        $return['image'] = 'METR_' . $current_timestamp . '.jpg';
        $return['start_datetime'] = $road_network->start_datetime;
        $return['end_datetime'] = $road_network->end_datetime;
